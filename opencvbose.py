...
def recognize_emotion():
    ...
    return emotions[max(set(predictions), key=predictions.count)]
...
def main():
    ...
    while True:
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'): #imshow expects a termination definition to work correctly, here it is bound to key 'q'
            break
        last_emotion = recognize_emotion()
        print(last_emotion)

        curl_command = 'curl -d "<play_info><app_key>3ZKYSw3X6GdOrAHh6F0SZDg225OiHKDg</app_key><url>http://192.168.1.10:8100/%s</url><service>service text</service><reason>reason text</reason><message>age text</message><volume>70</volume></play_info>" http://192.168.1.14:8090/speaker'
        happy = 'happy/walker.ogg'
        sad = 'sad/badDaySad.ogg'
        mad = 'angry/Glory.ogg'

        if current_emotion == 'happy': curl_command = curl_command % happy
        elif current_emotion == 'sad': curl_command = curl_command % sad
        elif current_emotion == 'angry': curl_command = curl_command % mad
